...
def comments():
    ...
    mongo.db.Comments.insert({"Street":regx,"Comment":da})
    street=db.Crimes.find({'STREET':regx}).limit(1)
    com=mongo.db.Comments.find({"Street":regx},{"Comment":1,"_id":0})
...
def map():
    ...
    return render_template('map.html', lat=lat,long=long,data=data,street=str,count=item_count)
